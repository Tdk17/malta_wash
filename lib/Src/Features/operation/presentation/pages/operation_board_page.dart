...
class _OperationBoardPageState extends State<OperationBoardPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final result = await Future.wait([
        ...
        repository.list(Endpoints.vehicles), repository.list(Endpoints.services),
      ]);
      if (!mounted) return;
      setState(() { appointments = result[0]; customers = result[1]; vehicles = result[2]; services = result[3]; });
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(height: 5), Text('Acompanhe cada veículo da chegada até a entrega.', style: TextStyle(color: Color(0xFF667085), fontSize: 14.5)),
        ])),
  ...
  Widget _appointmentCard(Map<String, dynamic> item, String status) {
    ...
    return Container(width: double.infinity, margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: const [BoxShadow(color: Color(0x0A101828), blurRadius: 16, offset: Offset(0, 7))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(width: double.infinity, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE2E8F0))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Icon(Icons.directions_car_filled_rounded, size: 17, color: Color(0xFF2563EB)), const SizedBox(width: 7), Expanded(child: Text(model, style: const TextStyle(fontWeight: FontWeight.w800, color: Color(0xFF334155))))]), const SizedBox(height: 5), Row(children: [const Icon(Icons.pin_outlined, size: 17, color: Color(0xFFFF6A00)), const SizedBox(width: 7), Text(plate, style: const TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF0F172A)))])])), const SizedBox(height: 8),
  ...
  Future<void> _advance(String id, String status) async {
    ...
    try {
      ...
      await _load();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(status == 'READY' ? 'Veículo marcado como pronto.' : status == 'COMPLETED' ? 'Atendimento finalizado.' : 'Atendimento atualizado.')));
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()))); }
    finally { if (mounted) setState(() => updatingId = null); }
  }

  Map<String, dynamic>? _resolveCustomer(Map<String, dynamic> appointment) { final embedded = appointment['customer']; if (embedded is Map) return embedded.map((k,v)=>MapEntry(k.toString(),v)); final customerId = _first(appointment,['customerId','clientId','userId','ownerId']); if(customerId.isNotEmpty){for(final c in customers){if(_id(c)==customerId)return c;}} final vehicle=_resolveVehicle(appointment); final ownerId=vehicle==null?'':_first(vehicle,['customerId','clientId','userId','ownerId']); if(ownerId.isNotEmpty){for(final c in customers){if(_id(c)==ownerId)return c;}} return null; }
  Map<String, dynamic>? _resolveVehicle(Map<String, dynamic> appointment) { final embedded=appointment['vehicle']; if(embedded is Map)return embedded.map((k,v)=>MapEntry(k.toString(),v)); final id=_first(appointment,['vehicleId','carId']); for(final v in vehicles){if(_id(v)==id)return v;} return null; }
  ...
  String _status(Map<String,dynamic> item){final raw=_first(item,['status','state','appointmentStatus']).toUpperCase(); if(raw=='PENDING_PAYMENT'||raw=='SCHEDULED'||raw=='BOOKED')return 'CONFIRMED'; if(raw=='ARRIVED')return 'CHECKED_IN'; if(raw=='WASHING')return 'IN_PROGRESS'; if(raw=='DONE'||raw=='FINISHED')return 'COMPLETED'; return statuses.contains(raw)?raw:'CONFIRMED';}
  ...
  String _shortTime(String raw){if(RegExp(r'^\d{2}:\d{2}').hasMatch(raw))return raw.substring(0,5);final dt=DateTime.tryParse(raw)?.toLocal();if(dt==null)return '';return '${dt.hour.toString().padLeft(2,'0')}:${dt.minute.toString().padLeft(2,'0')}';}
